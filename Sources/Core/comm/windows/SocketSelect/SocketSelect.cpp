...
bool SocketSelectBreaker::ReCreate() {
    ...
    if (INVALID_SOCKET == m_socket_w) {
        kflog_error2("INVALID_SOCKET with  m_socket_w=%d", WSAGetLastError());
    ...
    if (INVALID_SOCKET == m_socket_r) {
        kflog_error2("INVALID_SOCKET with m_socket_r=%d", WSAGetLastError());
    ...
    if (SOCKET_ERROR == Ret) {
        kflog_error2("ioctlsocket failed with m_socket_w: %d\n", WSAGetLastError());
    ...
    if (SOCKET_ERROR == Ret) {
        kflog_error2("ioctlsocket failed with m_socket_r: %d\n", WSAGetLastError());
        socket_close(m_socket_r);
        socket_close(m_socket_w);
        m_create_success = false;
        return m_create_success;
    ...
    if (SOCKET_ERROR == Ret) {
        kflog_error2("bind failed with m_socket_w: %d\n", WSAGetLastError());
        socket_close(m_socket_r);
        socket_close(m_socket_w);
        m_create_success = false;
        return m_create_success;
    ...
    if (SOCKET_ERROR == Ret) {
        kflog_error2("bind failed with m_socket_r: %d\n", WSAGetLastError());
        socket_close(m_socket_r);
        socket_close(m_socket_w);
        m_create_success = false;
        return m_create_success;
    ...
    if (SOCKET_ERROR == Ret) {
        kflog_error2("getsockname failed %d\n", WSAGetLastError());
        socket_close(m_socket_r);
        socket_close(m_socket_w);
        m_create_success = false;
        return m_create_success;
...
bool SocketSelectBreaker::Break() {
    ...
    if (ret < 0 || ret != strlen(dummy)) {
        kflog_error2(KFTSF"sendto Ret:%_, errno:(%_, %_)", ret, errno, WSAGetLastError());
        m_broken =  false;
        ReCreate();
    }
...
bool SocketSelectBreaker::Clear() {
    ...
    if (ret < 0) {
        kflog_error2(KFTSF"recvfrom Ret:%_, errno:(%_, %_)", ret, errno, WSAGetLastError());
        ReCreate();
        return false;
    }
...
void SocketSelect::PreSelect() {
    kflog_assert2(!IsBreak(), "Already break!");
    FD_ZERO(&readfd_);
    FD_ZERO(&writefd_);
    FD_ZERO(&exceptionfd_);
