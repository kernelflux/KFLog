...
void KFLogConsole(const KFLoggerInfo* _info, const char* _log) {
    Runtime2Cs_Comm::Singleton()->KFLogConsole((int)_info->level, char2PString(_info->tag), char2PString(_info->filename), char2PString(_info->func_name), _info->line, char2PString(_log));
}
