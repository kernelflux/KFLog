...
#include "comm/kflogger/loginfo_extract.h"
...
static KFLogNetworkStatus __GetNetworkStatus()
{
#if TARGET_OS_WATCH
    return ReachableViaWiFi;
#else
    return [KFLogReachability getCacheReachabilityStatus:NO];
#endif
}

void FlushReachability() {
#if !TARGET_OS_WATCH
   [KFLogReachability getCacheReachabilityStatus:YES];
#endif
}
...
//    CFURLRef url = (CFURLRef)[NSURL URLWithString: @"http://www.google.com"];
   ...
   if (0==CFStringCompare(http_type, kCFProxyTypeAutoConfigurationURL, 0))
   {
    ...
    {
        kflog_error2(KFTSF"convert error");
        return false;
    }

    strProxy = tmp_proxy;
    port = tmp_port;
    kflog_debug2(KFTSF"%0:%1", strProxy, port);
    return true;
#endif
}
...
bool getCurSIMInfo(SIMInfo& simInfo)
{
    ...
    kflog_verbose2(KFTSF"isp_code:%0", simInfo.isp_code);
...
bool getCurRadioAccessNetworkInfo(RadioAccessNetworkInfo& _raninfo)
{
    ...
    kflog_assert2(!_raninfo.radio_access_network.empty(), "%s", [currentRadioAccessTechnology UTF8String]);
