...
bool SocketPoll::Consign(SocketPoll& _consignor, bool _recover) {
    ...
    if (_recover) {
        ...
        kflog_assert2(it->events == _consignor.events_[0].events, KFTSF"%_ != %_", it->events, _consignor.events_[0].events);
        events_.erase(it, it+_consignor.events_.size());
    } else {
        kflog_assert2(it == events_.end());
        if (it != events_.end()) return false;
...
bool SocketPoll::ConsignReport(SocketPoll& _consignor, int64_t _timeout) const {
    ...
    kflog_assert2(find_it != events_.end());
    kflog_assert2(events_.end() - find_it >= (int)_consignor.events_.size());
    
    if (find_it == events_.end()) return false;
    
    for (auto &i : _consignor.events_) {
        kflog_assert2(i.fd == find_it->fd && i.events == find_it->events,
                 KFTSF"i(%_, %_), find_it(%_, %_)", i.fd, i.events, find_it->fd, find_it->events);
        if (0 != find_it->revents) {
            i.revents = find_it->revents;
            ++triggered_event_count;
            
            if (i.fd == _consignor.events_[0].fd) {
                kflog_assert2(&i == &(_consignor.events_[0]));
                continue;
            }
