...
bool SocketBreaker::ReCreate()
{
    ...
    int Ret;
    Ret = pipe(pipes_);
    kflog_assert2(-1 != Ret, "pipe errno=%d", errno);
    ...
    if (flags0 < 0 || flags1 < 0) {
        kflog_error2(KFTSF"get old flags error");
        close(pipes_[0]);
        close(pipes_[1]);
        pipes_[0] = -1;
        pipes_[1] = -1;
    ...
    if ((-1 == ret1) || (-1 == ret0)) {
        kflog_error2(KFTSF"fcntl error");
        close(pipes_[0]);
        close(pipes_[1]);
        pipes_[0] = -1;
        pipes_[1] = -1;
...
bool SocketBreaker::Break()
{
    ...
    {
        kflog_error2(KFTSF"Ret:%_, errno:(%_, %_)", ret, errno, strerror(errno));
        broken_ =  false;
    }
...
bool SocketBreaker::Clear()
{
    ...
    if (ret < 0)
    {
        kflog_verbose2(KFTSF"Ret=%0", ret);
        return false;
    }
