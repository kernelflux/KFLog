...
extern void log_formater(const KFLoggerInfo* _info, const char* _logbody, PtrBuffer& _log);
extern void KFLogConsole(const KFLoggerInfo* _info, const char* _log);

static KFLogAppenderMode sg_mode = kKFLogAppenderAsync;
...
static void __writetips2console(const char* _tips_format, ...) {
    ...
    KFLoggerInfo info;
    memset(&info, 0, sizeof(KFLoggerInfo));
...
static void __log2file(const void* _data, size_t _len, bool _move_file) {
    ...
    if (sg_cache_logdir.empty()) {
        if (__openlogfile(sg_logdir)) {
            ...
            if (kKFLogAppenderAsync == sg_mode) {
                __closelogfile();
            }
        }
        return;
    ...
    if (!write_sucess) {
        if (open_success && kKFLogAppenderSync == sg_mode) {
            __closelogfile();
        }
...
static void __kflog_appender_sync(const KFLoggerInfo* _info, const char* _log) {
...
static void __kflog_appender_async(const KFLoggerInfo* _info, const char* _log) {
    ...
    if (sg_log_buff->GetData().Length() >= kBufferBlockLength*1/3 || (NULL!=_info && kKFLogLevelFatal == _info->level)) {
...
void kflogger_appender(const KFLoggerInfo* _info, const char* _log) {
    ...
    if (sg_consolelog_open) KFLogConsole(_info,  _log);
    ...
    if (2 <= (int)recursion.Get() && NULL == s_recursion_str.get()) {
        ...
        KFLoggerInfo info = *_info;
        info.level = kKFLogLevelFatal;

        char recursive_log[256] = {0};
        snprintf(recursive_log, sizeof(recursive_log), "ERROR!!! kflogger_appender Recursive calls!!!, count:%d", (int)recursion.Get());
        ...
        KFLogConsole(&info,  strrecursion);
    } else {
        ...
        if (kKFLogAppenderSync == sg_mode)
            __kflog_appender_sync(_info, _log);
        else
            __kflog_appender_async(_info, _log);
    }
}
...
const char* kflogger_dump(const void* _dumpbuffer, size_t _len) {
...
static void get_mark_info(char* _info, size_t _infoLen) {
    ...
    snprintf(_info, _infoLen, "[%" PRIdMAX ",%" PRIdMAX "][%s]", kflogger_pid(), kflogger_tid(), tmp_time);
}
...
void kflog_appender_open(KFLogAppenderMode _mode, const char* _dir, const char* _nameprefix, const char* _pub_key) {
    ...
    kflogger_SetAppender(&kflogger_appender);
    ...
    kflogger_appender(NULL, appender_info);
    char logmsg[256] = {0};
    snprintf(logmsg, sizeof(logmsg), "get mmap time: %" PRIu64, (int64_t)get_mmap_time);
    kflogger_appender(NULL, logmsg);

    kflogger_appender(NULL, "KFLOG_URL: " KFLOG_URL);
    kflogger_appender(NULL, "KFLOG_PATH: " KFLOG_PATH);
    kflogger_appender(NULL, "KFLOG_REVISION: " KFLOG_REVISION);
    kflogger_appender(NULL, "KFLOG_BUILD_TIME: " KFLOG_BUILD_TIME);
    kflogger_appender(NULL, "KFLOG_BUILD_JOB: " KFLOG_TAG);
    ...
    BOOT_RUN_EXIT(kflog_appender_close);
...
#endif
    kflog_appender_open(_mode, _logdir.c_str(), _nameprefix, _pub_key);
...
void kflog_appender_setmode(KFLogAppenderMode _mode) {
    ...
    if (kKFLogAppenderAsync == sg_mode && !sg_thread_async.isruning()) {
...
void kflog_appender_setExtraMSg(const char* _msg, unsigned int _len) {
