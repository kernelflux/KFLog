...
void log_formater(const KFLoggerInfo* _info, const char* _logbody, PtrBuffer& _log) {
    ...
    if (NULL != _info) {
        ...
        int ret = snprintf((char*)_log.PosPtr(), 1024, "[%s][%s][%" PRIdMAX ", %" PRIdMAX "%s][%s][%s, %s, %d][",  // **CPPLINT SKIP**
                           _logbody ? levelStrings[_info->level] : levelStrings[kKFLogLevelFatal], temp_time,
